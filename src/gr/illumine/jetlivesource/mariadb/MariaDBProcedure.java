...
public class MariaDBProcedure extends Procedure{

	String returnString;

	MariaDBToJava mapper = new MariaDBToJava();
	
	
	public MariaDBProcedure() {
		super();
	}
	
	public MariaDBProcedure( Procedure proc ) {
		this();
 ...
 */
	public void ProcessDeclarationArguments() {
		...
		for (int i = 0; i < tokens.length; i++) {
			if (   StringUtils.emptyOrNull( tokens[i] ) || StringUtils.isWhiteSpace(tokens[i]) )
				continue;
			else if (tokens[i].equalsIgnoreCase(MariaDBToJava.IN) || tokens[i].equalsIgnoreCase(MariaDBToJava.OUT)  ) {
	...
	public void ProcessDeclarationReturnType() {
		if( !StringUtils.emptyOrNull(returnString) ){
			returnType.name = MariaDBToJava.RETURN_NAME;
			returnType.dbType = parseArgumentType(returnString);
			returnType.direction = MariaDBToJava.OUT;
		}else{
			returnType = null;
