...
public class MySqlEngine implements CodeGenerationEngine {
	Logger logger = Logger.getLogger(MySqlEngine.class.getName());

	LiveSourceConfiguration config;
	DatabaseProxy dbProxy;

	public MySqlEngine(LiveSourceConfiguration config) throws Exception {
		this.config = config;
	...
	public void createTablesViews() throws Exception {
		...
		try {
			...
			for (int i = 0; i < config.tables.size(); i++) {
				table = new MySqlTable(config.tables.get(i));
	...
	public void createProcedures() throws Exception {
		...
		try {
			...
			for (int i = 0; i < config.procedures.size(); i++) {
				procedure = new MySqlProcedure(config.procedures.get(i));
				...
				ProcedureToJava JavaConverter = new ProcedureToJava(
						config, procedure, new MySqlToJava());
	...
	public void createFunctions() throws Exception {
		...
		try {
			...
			// ----
			MySqlProcedure function = null;
			for (int i = 0; i < config.functions.size(); i++) {
				function = new MySqlProcedure(config.functions.get(i));
				...
				FunctionToJava JavaConverter = new FunctionToJava(
						config, function, new MySqlToJava());
...
// Private Routines
//----------------------------------	
	private String getSchema(MySqlTable table) {
		String clause = "";
